...
Eigen::Matrix2d ProjectionTdFactorAutoDiff::sqrt_info;
double ProjectionTdFactorAutoDiff::sum_t;
...
bool ProjectionTdFactor::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
    ...
    if (jacobians)
    {
        ...
        if (jacobians[0])
        {
            ...
            jaco_i.rightCols<3>() = ric.transpose() * Rj.transpose() * Ri * -math_utils::skewSymmetric(pts_imu_i);
        ...
        if (jacobians[1])
        {
            ...
            jaco_j.rightCols<3>() = ric.transpose() * math_utils::skewSymmetric(pts_imu_j);
        ...
        if (jacobians[2])
        {
            ...
            jaco_ex.rightCols<3>() = -tmp_r * math_utils::skewSymmetric(pts_camera_i) + math_utils::skewSymmetric(tmp_r * pts_camera_i) +
                math_utils::skewSymmetric(ric.transpose() * (Rj.transpose() * (Ri * tic + Pi - Pj) - tic));
...
void ProjectionTdFactor::check(double **parameters)
{
    ...
    for (int k = 0; k < 20; k++)
    {
        ...
        if (a == 0)
            ...
            Qi = Qi * math_utils::deltaQ(delta);
        else if (a == 2)
            Pj += delta;
        else if (a == 3)
            Qj = Qj * math_utils::deltaQ(delta);
        else if (a == 4)
            tic += delta;
        else if (a == 5)
            qic = qic * math_utils::deltaQ(delta);
        else if (a == 6 && b == 0)
            inv_dep_i += delta.x();
        else if (a == 6 && b == 1)
            td += delta.y();
