    ...
    for (auto it : p->sub_factors)
    {
        for (int i = 0; i < static_cast<int>(it->parameter_blocks.size()); i++)
        {
            ...
            if (size_i == 7)
            {
                ...
                PoseLocalParameterization::Global2LocalJacobian(it->parameter_blocks[i], &Jglobal_local);
            ...
            for (int j = i; j < static_cast<int>(it->parameter_blocks.size()); j++)
            {
                ...
                if (size_j == 7)
                {
                    size_j = 6;
                    Eigen::Matrix<double, 7, 6, Eigen::RowMajor> Jglobal_local;
                    PoseLocalParameterization::Global2LocalJacobian(it->parameter_blocks[j], &Jglobal_local);
        ...
        for (int i = 0; i < static_cast<int>(marginalization_info->keep_block_size.size()); i++)
        {
            if (jacobians[i])
            {
                ...
                if(size == 7)
                {
                    ...
                    jacobian.leftCols(size) = marginalization_info->linearized_jacobians.middleCols(idx, local_size)* jacobian_local2Global;
                }
            }
        }
    }
