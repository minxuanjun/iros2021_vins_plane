...
void ResidualBlockInfo::Evaluate() {
  ...
  cost_function->Evaluate(
      parameter_blocks.data(), residuals.data(), raw_jacobians);
  ...
  */
  ...
  for (int i = 0; i < NUM_THREADS; i++) {
    ...
    int ret = pthread_create(
        &tids[i], nullptr, ThreadsConstructA, (void*)&(threadsstruct[i]));
    if (ret != 0) {
  ...
  //  LOG(INFO) << GREEN << "Hpp: \n"
  //            << std::fixed
  //            << A.block<6, 6>(parameter_block_idx[newest_pose_addr],
  //                             parameter_block_idx[newest_pose_addr]);

  // ROS_DEBUG("thread summing up costs %f ms", t_thread_summing.toc());
  // ROS_INFO("A diff %f , b diff %f ", (A - tmp_A).sum(), (b -
  // tmp_b).sum());
...
std::vector<double*> MarginalizationInfo::getParameterBlocks(
  ...
  sum_block_size = std::accumulate(
      std::begin(keep_block_size), std::end(keep_block_size), 0);
    ...
    for (int i = 0;
      ...
      if (jacobians[i]) {
        ...
        if (size == 7) {
          ...
          PoseLocalParameterization::Local2GlobalJacobian(
              parameters[i], &jacobian_local2Global);
          jacobian.leftCols(size) =
              marginalization_info->linearized_jacobians.middleCols(
                  idx, local_size) *
              jacobian_local2Global;
        }
      }
    }
  }
