...
Eigen::Matrix2d ProjectionFactorAutoDiff::sqrt_information_;
  ...
  if (jacobians) {
    ...
    if (jacobians[0]) {
      ...
      jaco_i.rightCols<3>() = ric.transpose() * Rj.transpose() * Ri *
                              -math_utils::skewSymmetric(pts_imu_i);
    ...
    if (jacobians[1]) {
      ...
      jaco_j.rightCols<3>() =
          ric.transpose() * math_utils::skewSymmetric(pts_imu_j);
    ...
    if (jacobians[2]) {
      ...
      jaco_ex.rightCols<3>() =
          -tmp_r * math_utils::skewSymmetric(pts_camera_i) +
          math_utils::skewSymmetric(tmp_r * pts_camera_i) +
          math_utils::skewSymmetric(
              ric.transpose() * (Rj.transpose() * (Ri * tic + Pi - Pj) - tic));
