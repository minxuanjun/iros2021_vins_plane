...
void PoseLocalParameterization::Global2LocalJacobian(const double *x, Eigen::Matrix<double, 7, 6, Eigen::RowMajor>* jacobian) {
    Eigen::Map<const Eigen::Quaterniond> q(x+3);
    jacobian->setZero();
...
void PoseLocalParameterization::Local2GlobalJacobian(const double *x, Eigen::Matrix<double, 6, 7, Eigen::RowMajor> *jacobian)
{
    Eigen::Map<const Eigen::Quaterniond> q(x+3);
    jacobian->setZero();
    jacobian->topLeftCorner<3,3>().setIdentity();
