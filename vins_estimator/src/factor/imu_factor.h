 ...
 public:
  ...
  virtual bool Evaluate(double const* const* parameters,
    ...
    Eigen::Quaterniond Qj(
        parameters[2][6], parameters[2][3], parameters[2][4], parameters[2][5]);
    ...
    if (jacobians) {
      ...
      if (jacobians[0]) {
        ...
        Eigen::Matrix4d Jqi_inv_qi = math_utils::QuaternionInv2Quaternion(Qi);
        ...
        // Utility::skewSymmetric(Qi.inverse() * (0.5 * G * sum_dt * sum_dt + Pj
        // - Pi - Vi * sum_dt));

        jacobian_pose_i.block<3, 4>(O_P, O_R) =
            math_utils::QuaternionGlobalDiff(
                Qi.inverse(),
        ...
        Eigen::Quaterniond corrected_delta_q =
            ...
            math_utils::deltaQ(dq_dbg * (Bgi - pre_integration->linearized_bg));
        //                 jacobian_pose_i.block<3, 3>(O_R, O_R) =
        //                 -(Utility::Qleft(Qj.inverse() * Qi) *
        //                 Utility::Qright(corrected_delta_q)).topLeftCorner<3,
        //                 3>();

        //                residuals.block<3, 1>(O_R, 0) = 2 *
        //                (corrected_delta_q.inverse() * (Qi.inverse() *
        //                Qj)).vec();
        jacobian_pose_i.block<3, 4>(O_R, O_R) =
            2 *
            (math_utils::Qleft(corrected_delta_q.inverse()) *
             math_utils::Qright(Qj))
                .topLeftCorner<3, 4>() *
            Jqi_inv_qi;
        // todo 全局求导
//                jacobian_pose_i.block<3, 3>(O_R, O_R) =
//                -(Utility::Qleft(Qj.inverse() * Qi) *
//                Utility::Qright(corrected_delta_q)).bottomRows(3);
#endif
        // todo 全局求导
        jacobian_pose_i.block<3, 4>(O_V, O_R) =
            math_utils::QuaternionGlobalDiff(Qi.inverse(),
                                             G * sum_dt + Vj - Vi) *
            Jqi_inv_qi;
      ...
      if (jacobians[1]) {
        ...
        jacobian_speedbias_i.block<3, 3>(O_R, O_BG - O_V) =
            -math_utils::Qleft(Qj.inverse() * Qi * pre_integration->delta_q)
                 .topLeftCorner<3, 3>() *
            dq_dbg;
#endif
      ...
      if (jacobians[2]) {
        ...
        // todo 后面三行的求导
        //                jacobian_pose_j.block<3, 4>(O_R, O_R) =
        //                Utility::Qleft(corrected_delta_q.inverse() *
        //                Qi.inverse() * Qj).bottomRows(3);

        //                 jacobian_pose_j.block<3, 3>(O_R, O_R) =
        //                 Utility::Qleft(corrected_delta_q.inverse() *
        //                 Qi.inverse() * Qj).topLeftCorner<3, 3>();
        ...
        jacobian_pose_j.block<3, 4>(O_R, O_R) =
            2 * math_utils::Qleft(corrected_delta_q.inverse() * Qi.inverse())
                    .topLeftCorner<3, 4>();
#endif
