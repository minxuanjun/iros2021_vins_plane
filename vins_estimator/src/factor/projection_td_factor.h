...
class ProjectionTdFactorAutoDiff {
 public:
  ProjectionTdFactorAutoDiff(const Eigen::Vector3d& _pts_i,
                             const Eigen::Vector3d& _pts_j,
                             const Eigen::Vector2d& _velocity_i,
                             const Eigen::Vector2d& _velocity_j,
                             const double _td_i,
  ...
  bool operator()(const T* const pose_i,
    ...
    pts_i_td =
        pts_i.template cast<T>() -
        (td_est - static_cast<T>(td_i) +
         static_cast<T>(TR) / static_cast<T>(ROW) * static_cast<T>(row_i)) *
            velocity_i.template cast<T>();
    pts_j_td =
        pts_j.template cast<T>() -
        (td_est - static_cast<T>(td_j) +
         static_cast<T>(TR) / static_cast<T>(ROW) * static_cast<T>(row_j)) *
            velocity_j.template cast<T>();
    ...
    Eigen::Matrix<T, 3, 1> pts_camera_j =
        q_i_c.conjugate() * (pts_imu_j - t_i_c);

    Eigen::Map<Eigen::Matrix<T, 2, 1>> residual(residuals_ptr);

    T dep_j = pts_camera_j.z();
    residual =
        (pts_camera_j / dep_j).template head<2>() - pts_j_td.template head<2>();
  ...
  static ceres::CostFunction* Create(const Eigen::Vector3d& _pts_i,
                                     ...
                                     const double _row_j) {
    return new ceres::
        AutoDiffCostFunction<ProjectionTdFactorAutoDiff, 2, 7, 7, 7, 1, 1>(
            new ProjectionTdFactorAutoDiff(_pts_i,
                                           _pts_j,
                                           _velocity_i,
                                           _velocity_j,
                                           _td_i,
