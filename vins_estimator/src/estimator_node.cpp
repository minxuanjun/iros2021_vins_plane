...
void predict(const sensor_msgs::ImuConstPtr& imu_msg) {
  ...
  tmp_Q = tmp_Q * math_utils::deltaQ(un_gyr * dt);
    ...
    for (auto& measurement : measurements) {
      ...
      for (auto& imu_msg : measurement.first) {
        ...
        if (t <= img_t) {
          ...
          estimator.processIMU(
              dt_1, Vector3d(dx, dy, dz), Vector3d(rx, ry, rz));
      ...
      if (relo_msg != NULL) {
        ...
        estimator.setReloFrame(
            frame_stamp, frame_index, match_points, relo_t, relo_r);
      }
      ...
      Eigen::aligned_map<
          int,
          Eigen::aligned_vector<pair<int, Eigen::Matrix<double, 7, 1>>>>
          image;
...
int main(int argc, char** argv) {
  ...
  ros::Subscriber sub_imu = n.subscribe(
      IMU_TOPIC, 2000, imu_callback, ros::TransportHints().tcpNoDelay());
